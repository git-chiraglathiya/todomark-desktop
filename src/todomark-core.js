...
function formatDisplayFileName(filePath) {
  ...
  const rawName = normalizedPath.split("/").filter(Boolean).pop() || normalizedPath;
  const extIndex = rawName.lastIndexOf(".");
  const baseName =
    extIndex > 0 ? rawName.slice(0, extIndex) : rawName;
  const normalized = baseName.replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
  const fallback = rawName || String(filePath || "");
...
<html lang="en">
  <head>
    ...
    <style>
      :root {
        ...
        --scrollbar-size: 12px;
        --scrollbar-track: rgba(88, 103, 130, 0.12);
        --scrollbar-track-border: rgba(88, 103, 130, 0.2);
        --scrollbar-thumb: rgba(88, 103, 130, 0.42);
        --scrollbar-thumb-hover: rgba(88, 103, 130, 0.56);
        --scrollbar-thumb-active: rgba(88, 103, 130, 0.66);
        --scrollbar-thumb-border: rgba(255, 255, 255, 0.4);
      }
  ...
  <body>
    ...
    <script>
      const vscode = window.__TODOMARK_BRIDGE__;
      ...
      const applyTheme = ({ animate = true, persist = true } = {}) => {
        ...
        const tokens = {
          ...
          "--scrollbar-track": withAlpha(selected.accent, isDarkMode ? 0.22 : 0.15),
          "--scrollbar-track-border": withAlpha(
            selected.accentStrong,
            isDarkMode ? 0.34 : 0.24
          ),
          "--scrollbar-thumb": withAlpha(
            selected.accentStrong,
            isDarkMode ? 0.55 : 0.42
          ),
          "--scrollbar-thumb-hover": withAlpha(
            selected.accentStrong,
            isDarkMode ? 0.68 : 0.56
          ),
          "--scrollbar-thumb-active": withAlpha(
            selected.accentStrong,
            isDarkMode ? 0.78 : 0.66
          ),
          "--scrollbar-thumb-border": withAlpha(
            isDarkMode ? "#dbe5ff" : "#ffffff",
            isDarkMode ? 0.28 : 0.48
          ),
